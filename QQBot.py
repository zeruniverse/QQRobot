...
class group_thread(threading.Thread):
    ...
    def learn(self, key, value, needreply=True):
        ...
        if needreply:
            self.reply("学习成功！快对我说" + str(key) + "试试吧！（小黄鸡1.0）")
            self.save()
    ...
    def tucao(self, content):
        for key in self.replyList:
            if str(key) in content and self.replyList[key]:
                ...
                self.reply(self.replyList[key][rd]+"[由小黄鸡(1.0)复述，与本人无关]")
...
#modify followed group here
    GroupWatchList.append("1111111")
    GroupWatchList.append("11111111")
    GroupWatchList.append("111111111")
############################
    while 1:
